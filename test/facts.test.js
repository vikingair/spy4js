...
describe('Test-Utils', () => {
    ...
    it('does throw no exception if an comparison' + 'returns no difference in "equals"', () => {
        ...
        throws(() => equalsNot(firstObj, secondObj), { message: 'The given inputs were equal!' });
    });
    ...
    it('does not consider any own equals function' + 'when comparing with "equals"', done => {
        ...
        if (differenceOf(firstObj, secondObj, { useOwnEquals: true }) !== undefined) {
            throw new Error('"differenceOf was expected to' + ' use the own equals functions!"');
        }
        try {
            equals(firstObj, secondObj);
        } catch (expectedException) {
