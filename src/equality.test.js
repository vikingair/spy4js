...
describe('Spy - Equality', () => {
    it('should make an equality check for classes correctly', () => {
        ...
        expect(differenceOf(someInstance, someDifferentInstance)).toBe('--> attr3 / different date');
    });
    ...
    it('should detect flat differences correctly', () => {
        ...
        expect(differenceOf(null, 'test2')).toBe('null or undefined did not match');
        expect(differenceOf(undefined, null)).toBe('null or undefined did not match');
        // string
    ...
    it('should detect flat differences for classes correctly', () => {
        ...
        expect(differenceOf(new TestClass1('some String'), new TestClass2('some String'))).toBe('different constructor');
    });

    it('should flatly detect different keys length correctly', () => {
        expect(differenceOf([1, 2, 'test'], [1, 2, 'test'])).toBe(undefined);
        expect(differenceOf([1, 2, 'test'], [1, 'test'])).toBe('different key length');
    });
    ...
    it('should compare with defined equals key, if this exists', () => {
        ...
        expect(differenceOf(new TestClass(2), new TestClass(5))).toInclude('own equals method failed');
        expect(differenceOf(new TestClass(2), new TestClass(4), {useOwnEquals: false})).toBe('--> attr / different number');
    });
    ...
    it('should make deep equality checks correctly', () => {
        ...
        expect(differenceOf(obj1, obj2)).toBe('--> a / 1 / b / different string');
    });
});
